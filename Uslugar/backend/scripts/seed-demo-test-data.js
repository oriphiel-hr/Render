...
async function seedScenarioMatrix() {
  ...
  const jobPending = await prisma.job.create({
    data: {
      ...
      title: 'Sanacija vlage – jedan zid i podna lajsna (soba ~12 m²)',
      description:
        'Mikro sanacija nakon curenja uz prozor; premaz, zamjena PVC lajsne; nije cijeli stan. Split.',
      city: 'Split',
      budgetMin: 380,
      budgetMax: 720,
      status: 'OPEN',
  ...
  const jobApproved = await prisma.job.create({
    data: {
      userId: demoClient.id,
      title: 'Laminat u dvije sobe (~26 m²) – rad i podloga',
      description:
        'Demontaža starog poda, ugradnja podloge i laminata za dvije spavaće sobe; materijal uglavnom u budžetu. Zagreb.',
      city: 'Zagreb',
      budgetMin: 540,
      budgetMax: 920,
      status: 'OPEN',
  ...
  const completedJob = await prisma.job.create({
    data: {
      userId: demoClient.id,
      title: 'Čišćenje građevinske prašine – stan 60 m² (jednokratno)',
      description:
        'Temeljito čišćenje nakon manjih građevinskih radova; prašina, krhotine, prozori. Zagreb.',
      city: 'Zagreb',
      budgetMin: 220,
      budgetMax: 420,
      status: 'COMPLETED',
  ...
  if (directorProfileId) {
    const queueJob = await prisma.job.create({
      data: {
        userId: demoClient.id,
        title: 'Krov – zamjena oluka i popravak letve (jednoporod. kuća)',
        description:
          'Manji zahvat: novi oluk na jednoj strani krova, popravak letve; demo za LeadQueue (OFFERED/EXPIRED).',
        city: 'Zagreb',
        budgetMin: 320,
        budgetMax: 580,
        status: 'OPEN',
