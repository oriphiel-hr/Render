...
export async function recalculateTeamLocationStats(providerUserId) {
  ...
  for (const p of purchases) {
    const loc = findClosestTeamLocation(teamLocations, p.job);
    const lid = loc?.id ?? teamLocations.find(t => t.isPrimary)?.id ?? teamLocations[0]?.id;
    if (!lid) continue;
  ...
  for (const j of jobsWithAcceptedOffer) {
    ...
    if (!p) continue;
    const loc = findClosestTeamLocation(teamLocations, j);
    const lid = loc?.id ?? teamLocations.find(t => t.isPrimary)?.id ?? teamLocations[0]?.id;
